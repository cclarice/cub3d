...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/26 02:20:56 by cclarice          #+#    #+#             */
/*   Updated: 2021/03/01 19:29:01 by cclarice         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ver_line(t_eng *e, int x, int *draw, int color)
{
	while (draw[0] < draw[1])
	{
		put_pixel_to_img(&e->mdg, x, draw[0], color);
		draw[0]++;
	}
}

void	calc_line(t_eng *e, double walldist, int x, int type, double tex)
{
	...
	tex = 0;
	draw[0] = -height * 0.5 + e->rey * 0.5;
	draw[1] =  height * 0.5 + e->rey * 0.5;
	draw[0] < 0       ? draw[0] = 0 : 0;
	draw[1] >= e->rey ? draw[1] = e->rey - 1 : 0;
	if (type == 1)
		ver_line(e, e->rex - x, draw, 0xFFFF00 - 0x010100 * (int)(walldist * 16));
	else if (type == 2)
		ver_line(e, e->rex - x, draw, 0xFF0000 - 0x010000 * (int)(walldist * 16));
	else if (type == 3)
		ver_line(e, e->rex - x, draw, 0x00FFFF - 0x000101 * (int)(walldist * 16));
	else
		ver_line(e, e->rex - x, draw, 0xFF00FF - 0x010001 * (int)(walldist * 16));
}
...
void	raycasting(t_eng *e)
{
	...
	while (i < e->rex + e->fov / 2)
	{
		...
		if (r.vl[2] > r.hl[2])
			calc_line(e, r.hl[2] * fabs(cos((e->pla - a) / 2)), i, (a > P * 0.5 && a < P * 1.5) ? 2 : 3, r.hr[0]);
		else
			calc_line(e, r.vl[2] * fabs(cos((e->pla - a) / 2)), i, (a > P) ? 1 : 0, r.vr[1]);
		i++;
		a += one;
	}
	draw_dot(&(e->rma), e->plx * e->mmp.fullb, e->ply * e->mmp.fullb, 0xffffff);
